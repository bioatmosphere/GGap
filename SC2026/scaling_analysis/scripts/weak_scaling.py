...
- Fix grid_height (default 5), each rank owns a height x block_width column slab
- Width scales with GPU count: width = (sites_per_gpu / grid_height) * num_gpus
- Cross-rank edges per rank = height * 6 (constant for 2+ GPUs)
- All ranks do identical work

Usage:
    # 1 GPU
    srun -N1 -n1 --gpus-per-node=1 python weak_scaling.py

    # 4 GPUs
    srun -N1 -n4 --gpus-per-node=4 python weak_scaling.py

    # 8 GPUs with CSV output
    srun -N1 -n8 --gpus-per-node=8 python weak_scaling.py --csv ../results/weak_8gpus.csv
"""
...
# Add GGap to path
ggap_root = Path(__file__).resolve().parent.parent.parent.parent
...
def run_weak_scaling_test(args):
    ...
    timings = model._tick_timings  # list of dicts, one per tick
...
def main():
    parser = argparse.ArgumentParser(description="Weak scaling test")
