...
def main():
    ...
    strong_csv = csv_dir / "strong_scaling.csv"
    ...
    md.append(write_table_strong("Strong Scaling — 2,048 sites fixed", metrics_s, avg_s, gpus_s, "strong_scaling.csv"))
    ...
    md.append(f"| `strong_scaling.csv` | strong_scaling_speedup, phase_breakdown, §5, §6, "
    ...
    md.append("**To refresh:** drop new rows into the relevant CSV and re-run the affected plot "
              "scripts in `SC2026/figures/scripts/`, then re-run "
              "`python SC2026/figures/scripts/generate_results_md.py` to regenerate this document.\n")
