...
def partition_graph_metis(adjacency, site_ids, num_parts):
    ...
    gpmetis requires num_parts >= 2; for a single-rank (single-GPU) run we
    trivially assign every site to partition 0. If gpmetis is unavailable or
    fails (e.g. a degenerate near-empty regional graph), fall back to a
    round-robin assignment so the local reproduction run still proceeds.
    The multi-rank Frontier path with gpmetis present is unchanged.
    """
    # Single partition: no external partitioner needed.
    if num_parts <= 1:
        return {site_id: 0 for site_id in site_ids}
    ...
    try:
        ...
        if os.path.exists(partition_file):
            os.unlink(partition_file)
        log(f"  WARNING: gpmetis partitioning failed ({e}); "
            f"falling back to round-robin across {num_parts} parts")
        return {site_id: idx % num_parts for idx, site_id in enumerate(site_ids)}


def build_network_and_partition(data_dir, prefix, dispersal_factor, num_parts,
                                min_lat=None, max_lat=None,
                                min_lon=None, max_lon=None, site_ids=None):
    """
    Build dispersal network and partition sites.

    An optional geographic bounding box (min/max lat/lon) or explicit site-id
    list restricts the simulation to a regional subset of CONUS. When no bounds
    are given the full 1,424-site CONUS set is used (identical to the original
    Frontier run). Filtering is applied immediately after loading, so the
    dispersal graph and METIS partition are built over the subset only.
    ...
    # --- Regional / site-subset filter (local downscaled reproduction) ---
    use_box = any(v is not None for v in (min_lat, max_lat, min_lon, max_lon))
    if use_box or site_ids:
        id_set = set(site_ids) if site_ids else None

        def _keep(sid):
            if id_set is not None and sid not in id_set:
                return False
        ...
        kept = [sid for sid in all_site_ids if _keep(sid)]
        site_locations = {sid: site_locations[sid] for sid in kept}
        all_site_ids = kept
        box_desc = (f"lat[{min_lat},{max_lat}] lon[{min_lon},{max_lon}]"
                    if use_box else f"{len(id_set)} explicit ids")
        log(f"  Region filter active ({box_desc}): {len(all_site_ids)} sites selected")
        if not all_site_ids:
            raise ValueError("Region/site filter selected 0 sites — widen the bounds.")
...
def initialize_simulation(model, partition_map, directed_edges, site_locations,
    ...
    # Bulk-build this rank's local sites with globally-unique agent IDs, wire the
    # site/gap/tree hierarchy AND cross-rank dispersal (site->site), and load
    # everything via build_from_local_data. Cross-rank neighbors are declared so
    # SAGESim exchanges ghost data every tick.
    log(f"  Rank {rank}: building sites with {num_gaps} gaps, {maxtrees} maxtrees each...", force=True)

    local_sites = model.build_conus_local(
    ...
    log(f"  Rank {rank}: build_from_local_data completed in {t_init:.2f}s", force=True)
...
def run_simulation(model, local_sites, years, report_interval, output_dir, no_tree_data, no_snapshots=False):
    ...
    If no_snapshots=True, runs simulate(years) in a single call with no I/O,
    for measuring pure simulation cost.
    """
    log("\n" + "=" * 80)
    log(f"Phase 5: Running Simulation for {years} Years")
    log("-" * 80)

    # No-snapshot mode: single simulate() call, no I/O
    if no_snapshots:
        if rank == 0:
            log(f"\nNo-snapshot mode: running simulate({years}) in one call")
            log("")
        t_start = time.time()
        model.simulate(ticks=years, sync_workers_every_n_ticks=1)
        t_total = time.time() - t_start
        log("\n" + "=" * 80)
        log("Simulation Complete (no snapshots)")
        log(f"  Total time: {t_total:.2f}s")
        log(f"  Time per year: {t_total / years:.4f}s")
        log("=" * 80)
        return
    ...
    if rank == 0:
        ...
        log(f"  Post-processing: python extract_last_species_data.py --snapshot_dir {snapshot_dir} --metadata_dir {output_dir} --output_dir {output_dir}/last_year_species")
        log("")
    ...
    log(f"  To generate CSVs: python extract_last_species_data.py --snapshot_dir {snapshot_dir} --metadata_dir {output_dir} --output_dir {output_dir}/last_year_species")
    log("=" * 80)
...
def main():
    ...
    parser.add_argument("--data_dir", type=str,
                       default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "input_data"),
    ...
    parser.add_argument("--no_snapshots", action="store_true",
                       help="Skip periodic snapshots — runs simulate(years) in one call for pure timing")
    ...
    # Regional subset (local downscaled reproduction). Defaults None => full CONUS.
    parser.add_argument("--min_lat", type=float, default=None,
                       help="Southern latitude bound for regional subset (deg)")
    parser.add_argument("--max_lat", type=float, default=None,
                       help="Northern latitude bound for regional subset (deg)")
    parser.add_argument("--min_lon", type=float, default=None,
                       help="Western longitude bound for regional subset (deg, negative for W)")
    parser.add_argument("--max_lon", type=float, default=None,
                       help="Eastern longitude bound for regional subset (deg, negative for W)")
    parser.add_argument("--site_ids", type=int, nargs="+", default=None,
                       help="Explicit list of site IDs to simulate (overrides/adds to bbox)")
    ...
    # per-process seed (model.py:383), which makes runs irreproducible AND gives
    # each rank a different seed in a multi-rank run. The RNG is keyed on
    # logical_ids (partition-invariant), so a shared fixed seed makes 1-GPU and
    # N-GPU runs produce identical results.
