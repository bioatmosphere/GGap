...
def light_step(
    ...
    if is_alive == 1:
        ...
        # Get neighbor locations (spatial network)
        neighbor_ids = breed_tensor[agent_index]  # This should be locations, but keeping interface

        # Calculate shading from taller neighbors
        # Note: In production, this would use a spatial index
        # For now, iterate through all potential neighbors
        i = 0
        max_neighbors = 100  # Reasonable limit for performance
        while i < max_neighbors:
            # Check if we have a valid neighbor
            # In actual implementation, this would come from spatial indexing
            # Placeholder: check all agents within radius

            # For each neighbor, check if it's taller and within radius
            # If so, reduce light based on height difference

            i += 1

        # Apply species-specific light tolerance
        # Shade tolerance classes: 1=intolerant to 5=very tolerant
        # This affects how the tree responds to reduced light

        # Species parameters (hardcoded for GPU - would be from globals in production)
        # Species 1 (Red Maple): shade_tol=3
        # Species 2 (Loblolly Pine): shade_tol=2
        # Species 3 (White Oak): shade_tol=3
        # Species 4 (Sweetgum): shade_tol=2
        # Species 5 (Eastern Hemlock): shade_tol=5
        # Species 6 (Tulip Poplar): shade_tol=1

        shade_tol = 3  # Default moderate tolerance
        if species_id == 1:
            shade_tol = 3
        elif species_id == 2:
            shade_tol = 2
        ...
        # For initial implementation, use simple light reduction
        # In full model, this would integrate neighbor shading
        light_available = 1.0  # Will be updated with neighbor shading logic

        # Write light availability to write buffer
        set_this_agent_data_from_tensor(
            agent_index, write_light_avail_tensor, light_available
        )
...
def growth_step(
    ...
    growth_factor_tensor,
    write_age_tensor,
    write_diam_bht_tensor,
    write_forska_ht_tensor,
    write_biomC_tensor,
    write_growth_factor_tensor,
):
    """
    ...
    if is_alive == 1:
        ...
        # Write updates to write buffers
