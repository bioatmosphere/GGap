...
def step_func(
    ...
    """
    Changes in each time step regarding growth, mortality, and reproduction of different indiviudal trees (agents)
    in a forest as well as the changes in environmental conditions.

    Implementation notes:
    ---------------------
    This function is called by the simulation engine at each time step of the simulation. In a typical implementation,
    this function will iterate over all agents assigned to the current rank (in a parallel processing context) and for each agent,
    evaluate its current state and the states of its neighbors to determine if any state changes should occur. For example,

    This function accepts the current tick, the index of the agents/trees being evaluated,
    global parameters (e.g., temperature, precipitation, radiation, etc), etc.

    this function will call the growth, mortality, and reproduction functions.
