#!/usr/bin/env python3
"""Weak Scaling A — Throughput.

Standalone subfigure: aggregate throughput in billion agent-updates/second
vs. number of GPUs (log2 x-axis, log2 y-axis) for the communication-heavy
weak-scaling run (10 sites/GPU, 37.5% cross-rank). Includes ideal-linear line.
"""
import argparse
...
from _scaling_common import (
    FIG_W, FIG_H, COLOR_WEAK_A, COLOR_REF,
...
def plot(gpus, metrics, out_pdf, out_png, dpi):
    ...
    g_arr = np.array(gpus)
    tput = np.array([metrics[g]["throughput"] / 1e9 for g in gpus])  # B agent-updates/sec

    ideal_tput = (g_arr / gpus[0]) * tput[0]
    ax.plot(g_arr, ideal_tput, "--", color=COLOR_REF, linewidth=1, label="Ideal (linear)")
    ax.plot(g_arr, tput, "s-", color=COLOR_WEAK_A, linewidth=1.8, markersize=6, label="Measured")
...
def main():
    ...
    add_cli_args(parser, "weak_scaling_a_throughput")
    ...
    out_pdf = figs_dir / "weak_scaling_a_throughput.pdf"
    out_png = figs_dir / "weak_scaling_a_throughput.png"
