...
def read_csv(csv_path):
    ...
    for g in gpus_sorted:
        ...
        avg[g] = {}
        for key in runs[0]:
            avg[g][key] = sum(r.get(key, 0.0) for r in runs) / len(runs)

    return gpus_sorted, avg


def has_timing_breakdown(avg, gpus):
    """Check if CSV has per-tick timing breakdown columns."""
    return 'mean_gpu_compute' in avg[gpus[0]] and avg[gpus[0]]['mean_gpu_compute'] > 0


def main():
    parser = argparse.ArgumentParser(description="Plot weak scaling breakdown")
    parser.add_argument('--csv', type=str, default="../results/weak_scaling.csv",
                       help='Input CSV file')
    parser.add_argument('--output', type=str, default="../results/weak_scaling_breakdown.png",
                       help='Output PNG file')
    args = parser.parse_args()

    gpus_sorted, avg = read_csv(args.csv)
    ...
    has_breakdown = has_timing_breakdown(avg, gpus_sorted)
    ncols = 4 if has_breakdown else 3
    ...
    labels = ['Site Init', 'GPU Setup', 'First Tick', 'Steady State', 'Load Globals', 'Connectivity']
    ...
    # Panel 4: Compute vs Communication breakdown (if available)
    if has_breakdown:
        ax = axes[3]

        # GPU execution = kernel launch (gpu_compute) + kernel completion wait (gpu_sync)
        gpu_execution = np.array([(avg[g].get("mean_gpu_compute", 0) + avg[g].get("mean_gpu_sync", 0)) * 1000 for g in gpus])
        data_prep = np.array([avg[g].get("mean_data_prep", 0) * 1000 for g in gpus])
        mpi_pack = np.array([avg[g].get("mean_mpi_gpu_pack", 0) * 1000 for g in gpus])
        mpi_exchange = np.array([avg[g].get("mean_mpi_exchange", 0) * 1000 for g in gpus])
        mpi_unpack = np.array([avg[g].get("mean_mpi_gpu_unpack", 0) * 1000 for g in gpus])
        write_back = np.array([avg[g].get("mean_write_back", 0) * 1000 for g in gpus])
        kern_args = np.array([avg[g].get("mean_kernel_args_build", 0) * 1000 for g in gpus])

        bottom = np.zeros(len(gpus))
        bar_data = [
            (gpu_execution, '#2196F3', 'GPU Execution'),
            (mpi_pack, '#FF5722', 'MPI Pack'),
            (mpi_exchange, '#F44336', 'MPI Exchange'),
            (mpi_unpack, '#E91E63', 'MPI Unpack'),
            (data_prep, '#FF9800', 'Data Prep'),
            (write_back, '#4CAF50', 'Write Back'),
            (kern_args, '#607D8B', 'Kernel Args'),
        ]

        for d, c, l in bar_data:
            ax.bar(gpu_labels, d, bottom=bottom, color=c, label=l, edgecolor='white', linewidth=0.5)
            bottom += d

        ax.set_xlabel("Number of GPUs", fontsize=12)
        ax.set_ylabel("Time per Tick (ms)", fontsize=12)
        ax.set_title("Per-Tick Compute vs Communication", fontsize=14)
        ax.legend(loc='upper left', fontsize=8)
        ax.grid(axis='y', alpha=0.3)

        # Print compute/comm fractions
        mpi_frac = np.array([avg[g].get("mpi_fraction", 0) for g in gpus])
        gpu_exec_frac = np.array([avg[g].get("gpu_execution_fraction", 0) for g in gpus])
    ...
    header = f"{'GPUs':>6} {'Sites':>7} {'Init(s)':>8} {'GPU(s)':>7} {'1stTick':>8} {'Steady':>8} {'SimTotal':>9} {'Tick(ms)':>9} {'Eff%':>6}"
    if has_breakdown:
        header += f" {'GPU Exec%':>9} {'MPI%':>6}"
    print(header)
    print("-" * len(header))
    for i, g in enumerate(gpus):
        eff = baseline_sim / sim_time[i] * 100
        total_sites = int(avg[g]["total_sites"])
        line = f"{g:>6} {total_sites:>7} {site_init[i]:>8.1f} {gpu_setup[i]:>7.1f} {first_tick[i]:>8.1f} {steady_state[i]:>8.1f} {sim_time[i]:>9.1f} {mean_tick[i]*1000:>9.2f} {eff:>5.1f}%"
        if has_breakdown:
            line += f" {avg[g].get('gpu_execution_fraction', 0)*100:>8.1f}% {avg[g].get('mpi_fraction', 0)*100:>5.1f}%"
        print(line)
